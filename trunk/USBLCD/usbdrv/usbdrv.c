...
{
	uchar   len  		= 0, 
			*dataPtr 	= usbTxBuf + 9;  /* здесь 2 байта свободного пространства в конце буфера */
 ...
 */
    ...
    if(usbTxLen & 0x10)
	{    /* передача system idle */
        if(usbMsgLen != USB_NO_MSG)
		{    /* данные для передачи в ожидании? */
            usbBuildTxBlock();
        }
    }
    for(i = 10; i > 0; i--)
    ...
    if(i == 0)
	{ /* состояние RESET, вызывается несколько раз во время сброса */
        usbNewDeviceAddr = 0;
        usbDeviceAddr = 0;
        usbResetStall();
    }
