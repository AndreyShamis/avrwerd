...
void CsetledguiDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHECK2, ledOn2);
}
...
void CsetledguiDlg::OnBnClickedCheck2()
{
    CString msg;
    ...
    isOn2 = (ledOn2.GetCheck() == BST_CHECKED)? 0 : 1;
    cnt = usb_control_msg(handle, USB_TYPE_VENDOR | USB_RECIP_DEVICE | USB_ENDPOINT_OUT, CUSTOM_RQ_SET_STATUS10, isOn2, 0, buffer, 0, 5000);
    if(cnt < 0)
    {
        //fprintf(stderr, "USB error: %s\n", usb_strerror());
        msg.Format("USB error: %s\n", usb_strerror());
